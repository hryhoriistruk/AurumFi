// FileName: /Backend/src/network/p2p.rs
...
pub struct P2PNode {
    ...
    pub listening_port: u16, // Додаємо порт, на якому слухає нода
}

impl P2PNode {
    pub fn new(blockchain: Arc<NeoBlockchain>, listening_port: u16) -> Self {
        Self {
    ...
    pub async fn start(&self, port: u16) -> tokio::io::Result<()> {
        ...
        loop {
            ...
            let _current_node_port = self.listening_port; // Позначено як неактивна змінна, щоб уникнути попередження
    ...
    pub async fn connect_to_peer(&self, address: &str) -> tokio::io::Result<()> {
        if let Ok(mut stream) = TcpStream::connect(address).await {
            let peer_msg = json!({
                "type": "peer",
                "data": format!("127.0.0.1:{}", self.listening_port) // Використовуємо збережений порт
            });
