// FileName: /Backend/src/main.rs
use aurumfi_neo_token::{NeoBlockchain, AppConfig, KeypairManager, AurumFiToken, P2PNode};
use aurumfi_neo_token::blockchain::Transaction; // Використовуємо Transaction з blockchain.rs
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Виправлено: додано другий аргумент - listening_port
    let p2p_node = P2PNode::new(Arc::new(blockchain.clone()), 6000);
    ...
    match cli.command {
        ...
        Commands::Send { from, to, amount, fee: _ } => {
            ...
            // Виправлено: додано поле public_key (порожнє, бо CLI не підписує)
            let tx = Transaction {
                sender: from.clone(),
                recipient: to.clone(),
                amount: parsed_amount,
                signature: "CLI_UNSIGNED".to_string(), // Позначаємо як непідписану з CLI
                public_key: "".to_string(), // Додано, щоб компілювалось
            };

            if blockchain.create_transaction(tx) {
                println!("✅ Transaction created successfully (unsigned from CLI)! It will be mined if valid.");
                println!("📤 From: {}", from);
                println!("📥 To: {}", to);
                ...
                println!("❌ Transaction rejected: insufficient balance or invalid.");
            }
        }
